"""09-mots
Permet de manipuler les mots de la langue française à partir
d'un corpus textuel et de réaliser des recherches simples ou complexes.
"""
...
def cherche2(
    ...
    Returns:
        set: retourne le sous ensemble des mots commençant par une
        chaine présente dans lstart, contenant une chaine présente
        dans lmid et finissant par une chaine présente dans lstop,
    ...
    for mot in mots:
        if not nmin <= len(mot) <= nmax:
            continue
        if not any(mot.startswith(pfx) for pfx in lstart):
            continue
        if not any(mot.endswith(sfx) for sfx in lstop):
            continue
...
#     # ['bachi-bouzouks', 'bayadères', 'coloquintes',
#           'ectoplasmes', 'macchabées', 'oryctéropes', 'zouaves']
